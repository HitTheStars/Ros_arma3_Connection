...
### 🎯 推荐架构：深度估计方案（2025-11-03）

**使用 Depth Anything V2 从 RGB 图像生成深度图，实现完整的感知-规划-控制闭环。**

#### 为什么需要深度估计？

对于 **Arma 3 Altis 地图**（270 km²）的灾害救援场景：

- ❌ **预加载全地图点云不可行**：数据量将达到 TB 级别，内存和磁盘无法承载
- ❌ **静态地图不适用**：灾害现场有动态障碍物（崩塔、火灾、移动的车辆）
- ✅ **实时深度感知是必需的**：提供局部环境信息，支持动态建图

#### 技术优势

- ✅ **GPU 加速深度估计**：RTX 4060 + DirectML，推理时间仅 12ms
- ✅ **端到端低延迟**：< 50ms，支持 10 FPS 实时运行
- ✅ **单目深度估计**：不需要双目相机或 LiDAR，简单可靠
- ✅ **完整闭环**：感知（深度）→ 规划（EGO-Planner）→ 控制（Arma 3）

#### 适用场景

- ✅ **灾害救援**：动态环境的实时障碍物检测
- ✅ **大范围地图**：不需要预加载整个地图，仅处理局部区域
- ✅ **未知环境探索**：实时建图和路径规划
- ✅ **多无人机协同**：每架无人机独立感知环境
...
**只使用 ArmaCOM 进行状态和控制通信，不包含深度感知。**

**限制**：
- ❌ 仅适用于**小范围室内环境**（< 100m²）
- ❌ 需要**预加载静态点云**，不适用于 Altis 大地图
- ❌ **无法检测动态障碍物**，不适用于灾害救援场景

**仅用于**：
- 快速验证 ROS 通信是否正常
- 测试 EGO-Planner 的控制逻辑
- 学习和理解系统架构
...
> ⚠️ **重要**：以下是**简化版**启动流程。对于完整的深度估计架构，请查看 [快速启动指南](QUICK_START_GUIDE.md)。
