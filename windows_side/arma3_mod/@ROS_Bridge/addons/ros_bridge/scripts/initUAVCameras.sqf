/*
    UAV Camera Initialization Script
    无人机摄像头初始化脚本
    
    功能：
    1. 为每个无人机创建摄像头
    2. 使用 Render-to-Texture (R2T) 将画面渲染到屏幕
    3. 布局：6 个无人机，3列×2行
    
    基于: codingWithArma3 项目
    作者: Manus AI
    日期: 2025-11-03
*/
...
_screenHeight = safeZoneH / 3;    // 每行高度（1/3 屏幕高）

// 初始化每个无人机的摄像头和显示框
{
    private _uav = _x;                // 当前无人机
    private _index = _forEachIndex;   // 当前索引（0 到 5）
    ...
    // 计算框的坐标和大小
    private _boxWidth = _screenWidth / 3; // 每个框占屏幕的 1/3 宽度
